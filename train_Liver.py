...
import time
from networks.recursive_cascade_networks import RecursiveCascadeNetwork, mask_metrics, jacobian_det_var, \
    neg_jacobian_det
...
dataset_opt = {"dataroot": "../datasets/combine",
               "batch_size": 1,
               "use_shuffle": True,
               "num_workers": 4,
...
def main(args):
    ...
    dataset = Data.create_dataset_Liver(dataroot=dataset_opt["dataroot"],
    ...
    if args.T_name:
        Tparams = args.T_name.split("_", maxsplit=2)
        Tnetwork = Tparams[1]
        print(Tnetwork, args.T_epo)

        Teacher = RecursiveCascadeNetwork(n_cascades=args.T_cas,
                                          im_size=dataset_opt["finesize"],
                                          network=Tnetwork).cuda()
        params_dict = torch.load(
            "./logs/{}/model_wts/{}.pth".format(args.T_name, args.T_epo))
        Teacher.eval()
    ...
    if args.strategy == "plain":
        ...
        S_trainable_params = []
        for submodel in model.stems:
            S_trainable_params += list(submodel.parameters())

        S_optim = Adam(S_trainable_params, lr=1e-4)
        S_scheduler = MultiStepLR(optimizer=S_optim, milestones=[training_iters * 0.6, training_iters * 0.8],
                                  gamma=0.5)
    elif args.strategy == "KD":
        S_trainable_params = []
        for submodel in model.stems:
            S_trainable_params += list(submodel.parameters())
    ...
    for curr_epoch in range(1, args.epochs + 1):
        for istep, train_data in enumerate(train_loader, start=1):
            ...
            if args.strategy == "plain":
                ...
                scheduler.step()
            elif args.strategy in "KD":
                print("LR:", S_scheduler.get_last_lr())
                S_com = (model, S_optim, reconstruction)
                hyper = {
                    "gamma": 0.8,
                }
                rec_loss_mean, flows_loss_mean = KD(S_com, Teacher, datas, hyper)
                if record_cnter % args.record_freq == 0:
                    writer.add_scalar(tag="Loss/reconstruction",
                                      scalar_value=rec_loss_mean,
                                      global_step=record_cnter)
                    writer.add_scalar(tag="Loss/flow_dist",
                                      ...
                                      global_step=record_cnter)
                print(
                    "Rec loss: {}, flow loss: {}".format(round(rec_loss_mean, 4), round(flows_loss_mean, 4),
                                                         ))
                S_scheduler.step()
            ...
            if record_cnter == 1 or record_cnter % args.val_freq == 0:
                ...
                with torch.no_grad():
                    dataset = Data.create_dataset_Liver(dataroot="../datasets/sliver",
                                                        finesize=dataset_opt["finesize"],
                                                        phase="val")
                    ...
                    for idx, val_data in enumerate(val_loader):
                        ...
                        for flow in flows:
                            ...
                            dice_score, jacc_score = mask_metrics(fixed_seg, warped_seg)
                            jacobi_det_var = jacobian_det_var(flow)
                            neg_jacobi = neg_jacobian_det(flow)
                            dsc_blocks.append(dice_score.item())
                            jacc_blocks.append(jacc_score.item())
                ...
                val_dsc = np.mean(val_epoch_dsc, axis=0)
                val_jacc = np.mean(val_epoch_jacc, axis=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ckp_freq", type=int, default=50000)
    parser.add_argument("--val_freq", type=int, default=50000)
    parser.add_argument("--record_freq", type=int, default=8)
    ...
    parser.add_argument("--T_epo", type=str)
    parser.add_argument("--T_cas", type=int)  # 4
    parser.add_argument("--strategy", type=str,
                        help="/Fitnet/plain/selfdistill/twindistill/SDH_like")
