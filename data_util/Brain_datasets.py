...
class BrainDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label_dataA = sitk.GetArrayFromImage(label_dataA).astype(np.float32)
        label_dataB = dataPath['label_moving']
        label_dataB = sitk.ReadImage(label_dataB)
        label_dataB = sitk.GetArrayFromImage(label_dataB).astype(np.float32)
        ...
        return {'M': moving, 'F': fixed, 'MS': movingM, 'FS': fixedM, 'Index': index,
                'name': dataPath['image_moving'].split('/')[-1] + "->" + dataPath['image_fixed'].split('/')[-1]}
